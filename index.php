...
if (empty($_GET["search"])){
	$sql_prod = "SELECT pr.ID, pr.category_id, pr.color, pr.dimensions, pr.description, pr.price, pr.product_name, pr.product_image, pr.qty
	FROM products AS pr JOIN categories AS ca ON ca.ID = pr.category_id ORDER BY pr.product_name ASC";
} else{
	$sql_prod = "SELECT pr.ID, pr.category_id, pr.color, pr.dimensions, pr.description, pr.price, pr.product_name, pr.product_image, pr.qty
	FROM products AS pr JOIN categories AS ca ON ca.ID = pr.category_id WHERE pr.product_name LIKE ? ORDER BY pr.product_name ASC";
}
?>
